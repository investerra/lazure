...
var commandMetadata = map[string]commandMeta{
	...
	"lazure wait-for-deploy": {
		useCase:       "after deploy, verify the public app route is serving the expected build by polling a JSON version endpoint.",
		prerequisites: []string{prereqAzureAuth, prereqAppLive, "The app exposes a JSON endpoint such as `/version` with a commit field."},
		dependencies:  []string{depAzureARM, "HTTP access to the app custom domain or Container Apps FQDN."},
	},
	"lazure scale": {
