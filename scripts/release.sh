...
# notify sends an OS-aware desktop notification (notify-send on Linux,
# terminal-notifier/osascript on macOS), falling back to stdout.
notify() {
    local status="$1" tag="$2" title body
    case "$status" in
